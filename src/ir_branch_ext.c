                               ...
                               const struct amivm_cpu_state *cpu)
{
    ...
    if (op->ea_mode == AMIVM_IR_EA_D16_AN)
        return cpu->a[op->reg] + (uint32_t)op->imm;
    if (op->ea_mode == AMIVM_IR_EA_PC_D16)
        return op->guest_pc + 2u + (uint32_t)op->imm;
                          ...
                          const uint16_t *words, size_t word_count)
{
    ...
    /* M2.49: d16(An) and d16(PC) control transfers. */
    if ((branch_word & 0xfff8u) == 0x4ea8u ||
        (branch_word & 0xfff8u) == 0x4ee8u) {
