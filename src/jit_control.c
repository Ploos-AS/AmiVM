...
typedef int (*helper_ctx_u32_u32_u32)(struct amivm_jit_context *,uint32_t,uint32_t,uint32_t);
...
HELPER_ADDRESS(jsr_full_indexed_helper_address,helper_ctx_u32_u32_u32,amivm_jit_helper_jsr_full_indexed)
HELPER_ADDRESS(jmp_full_indexed_helper_address,helper_ctx_u32_u32_u32,amivm_jit_helper_jmp_full_indexed)
...
static uint32_t encode_full_indexed(const struct amivm_ir_op *op){return((uint32_t)op->index_reg)|((uint32_t)op->index_is_addr<<3u)|((uint32_t)op->index_long<<4u)|(scale_shift(op->index_scale)<<5u)|((op->ea_mode==AMIVM_IR_EA_PC_D8_XN?1u:0u)<<7u)|((uint32_t)op->reg<<8u)|((uint32_t)op->base_suppress<<11u)|((uint32_t)op->index_suppress<<12u);}
static int compile_jsr(const struct amivm_ir_block *block,struct amivm_jit_code *code){const struct amivm_ir_op *op=&block->ops[block->op_count-1u];uint64_t helper;uint32_t target,third=0u;int three_args=0;int rc;if(op->ea_mode==AMIVM_IR_EA_AN){if(op->reg>=8u)return AMIVM_JIT_UNSUPPORTED;rc=jsr_helper_address(&helper);target=(uint32_t)op->reg;}else if(op->ea_mode==AMIVM_IR_EA_D16_AN){if(op->reg>=8u)return AMIVM_JIT_UNSUPPORTED;rc=jsr_d16_helper_address(&helper);target=encode_d16_an(op);}else if(op->ea_mode==AMIVM_IR_EA_D8_AN_XN||op->ea_mode==AMIVM_IR_EA_PC_D8_XN){if(op->full_format){if(op->indirect_mode!=AMIVM_EA_INDIRECT_NONE)return AMIVM_JIT_UNSUPPORTED;rc=jsr_full_indexed_helper_address(&helper);target=encode_full_indexed(op);third=(uint32_t)op->base_displacement;three_args=1;}else{rc=jsr_indexed_helper_address(&helper);target=encode_indexed(op);}}else if(op->ea_mode==AMIVM_IR_EA_PC_D16){rc=bsr_helper_address(&helper);target=op->guest_pc+2u+(uint32_t)op->imm;}else if(op->ea_mode==AMIVM_IR_EA_ABS_W||op->ea_mode==AMIVM_IR_EA_ABS_L){rc=bsr_helper_address(&helper);target=(uint32_t)op->imm;}else return AMIVM_JIT_UNSUPPORTED;if(rc!=AMIVM_JIT_OK)return rc;rc=prepare_prefix(block,code);if(rc!=AMIVM_JIT_OK)return rc;rc=emit_context(code);if(rc!=AMIVM_JIT_OK)return rc;rc=emit_imm32(code,0xbeu,op->guest_pc+op->instruction_bytes);if(rc!=AMIVM_JIT_OK)return rc;rc=emit_imm32(code,0xbau,target);if(rc!=AMIVM_JIT_OK)return rc;if(three_args){rc=emit_imm32(code,0xb9u,third);if(rc!=AMIVM_JIT_OK)return rc;}rc=emit_call_tail(code,helper);if(rc!=AMIVM_JIT_OK)return rc;finish_control_code(block,code);return AMIVM_JIT_OK;}
static int compile_jmp(const struct amivm_ir_block *block,struct amivm_jit_code *code){const struct amivm_ir_op *op=&block->ops[block->op_count-1u];uint64_t helper;uint32_t arg,second=0u,third=0u;int args=1;int rc;if(op->ea_mode==AMIVM_IR_EA_AN){if(op->reg>=8u)return AMIVM_JIT_UNSUPPORTED;rc=jmp_helper_address(&helper);arg=(uint32_t)op->reg;}else if(op->ea_mode==AMIVM_IR_EA_D16_AN){if(op->reg>=8u)return AMIVM_JIT_UNSUPPORTED;rc=jmp_d16_helper_address(&helper);arg=encode_d16_an(op);}else if(op->ea_mode==AMIVM_IR_EA_D8_AN_XN||op->ea_mode==AMIVM_IR_EA_PC_D8_XN){arg=op->guest_pc+2u;if(op->full_format){if(op->indirect_mode!=AMIVM_EA_INDIRECT_NONE)return AMIVM_JIT_UNSUPPORTED;rc=jmp_full_indexed_helper_address(&helper);second=encode_full_indexed(op);third=(uint32_t)op->base_displacement;args=3;}else{rc=jmp_indexed_helper_address(&helper);second=encode_indexed(op);args=2;}}else if(op->ea_mode==AMIVM_IR_EA_PC_D16){rc=jmp_abs_helper_address(&helper);arg=op->guest_pc+2u+(uint32_t)op->imm;}else if(op->ea_mode==AMIVM_IR_EA_ABS_W||op->ea_mode==AMIVM_IR_EA_ABS_L){rc=jmp_abs_helper_address(&helper);arg=(uint32_t)op->imm;}else return AMIVM_JIT_UNSUPPORTED;if(rc!=AMIVM_JIT_OK)return rc;rc=prepare_prefix(block,code);if(rc!=AMIVM_JIT_OK)return rc;rc=emit_context(code);if(rc!=AMIVM_JIT_OK)return rc;rc=emit_imm32(code,0xbeu,arg);if(rc!=AMIVM_JIT_OK)return rc;if(args>=2){rc=emit_imm32(code,0xbau,second);if(rc!=AMIVM_JIT_OK)return rc;}if(args>=3){rc=emit_imm32(code,0xb9u,third);if(rc!=AMIVM_JIT_OK)return rc;}rc=emit_call_tail(code,helper);if(rc!=AMIVM_JIT_OK)return rc;finish_control_code(block,code);return AMIVM_JIT_OK;}
