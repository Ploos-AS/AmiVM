...
static uint32_t encode_indexed(const struct amivm_ir_op *op){uint32_t shift=0u,s=(uint32_t)op->index_scale;while(s>1u){++shift;s>>=1u;}return((uint32_t)op->imm&0xffu)|((uint32_t)op->index_reg<<8u)|((uint32_t)op->index_is_addr<<11u)|((uint32_t)op->index_long<<12u)|(shift<<13u)|((op->ea_mode==AMIVM_IR_EA_PC_D8_XN?1u:0u)<<15u)|((uint32_t)op->reg<<16u);}
static int compile_jsr(const struct amivm_ir_block *block,struct amivm_jit_code *code){const struct amivm_ir_op *op=&block->ops[block->op_count-1u];uint64_t helper;uint32_t target;int rc;if(op->ea_mode==AMIVM_IR_EA_AN){if(op->reg>=8u)return AMIVM_JIT_UNSUPPORTED;rc=jsr_helper_address(&helper);target=(uint32_t)op->reg;}else if(op->ea_mode==AMIVM_IR_EA_D16_AN){if(op->reg>=8u)return AMIVM_JIT_UNSUPPORTED;rc=jsr_d16_helper_address(&helper);target=encode_d16_an(op);}else if(op->ea_mode==AMIVM_IR_EA_D8_AN_XN||op->ea_mode==AMIVM_IR_EA_PC_D8_XN){rc=jsr_indexed_helper_address(&helper);target=encode_indexed(op);}else if(op->ea_mode==AMIVM_IR_EA_PC_D16){rc=bsr_helper_address(&helper);target=op->guest_pc+2u+(uint32_t)op->imm;}else if(op->ea_mode==AMIVM_IR_EA_ABS_W||op->ea_mode==AMIVM_IR_EA_ABS_L){rc=bsr_helper_address(&helper);target=(uint32_t)op->imm;}else return AMIVM_JIT_UNSUPPORTED;if(rc!=AMIVM_JIT_OK)return rc;rc=prepare_prefix(block,code);if(rc!=AMIVM_JIT_OK)return rc;rc=emit_context(code);if(rc!=AMIVM_JIT_OK)return rc;rc=put8(code,0xbeu);if(rc!=AMIVM_JIT_OK)return rc;rc=put32(code,op->guest_pc+op->instruction_bytes);if(rc!=AMIVM_JIT_OK)return rc;rc=put8(code,0xbau);if(rc!=AMIVM_JIT_OK)return rc;rc=put32(code,target);if(rc!=AMIVM_JIT_OK)return rc;rc=emit_call_tail(code,helper);if(rc!=AMIVM_JIT_OK)return rc;finish_control_code(block,code);return AMIVM_JIT_OK;}
static int compile_jmp(const struct amivm_ir_block *block,struct amivm_jit_code *code){const struct amivm_ir_op *op=&block->ops[block->op_count-1u];uint64_t helper;uint32_t arg,second=0u;int two_args=0;int rc;if(op->ea_mode==AMIVM_IR_EA_AN){if(op->reg>=8u)return AMIVM_JIT_UNSUPPORTED;rc=jmp_helper_address(&helper);arg=(uint32_t)op->reg;}else if(op->ea_mode==AMIVM_IR_EA_D16_AN){if(op->reg>=8u)return AMIVM_JIT_UNSUPPORTED;rc=jmp_d16_helper_address(&helper);arg=encode_d16_an(op);}else if(op->ea_mode==AMIVM_IR_EA_D8_AN_XN||op->ea_mode==AMIVM_IR_EA_PC_D8_XN){rc=jmp_indexed_helper_address(&helper);arg=op->guest_pc+2u;second=encode_indexed(op);two_args=1;}else if(op->ea_mode==AMIVM_IR_EA_PC_D16){rc=jmp_abs_helper_address(&helper);arg=op->guest_pc+2u+(uint32_t)op->imm;}else if(op->ea_mode==AMIVM_IR_EA_ABS_W||op->ea_mode==AMIVM_IR_EA_ABS_L){rc=jmp_abs_helper_address(&helper);arg=(uint32_t)op->imm;}else return AMIVM_JIT_UNSUPPORTED;if(rc!=AMIVM_JIT_OK)return rc;rc=prepare_prefix(block,code);if(rc!=AMIVM_JIT_OK)return rc;rc=emit_context(code);if(rc!=AMIVM_JIT_OK)return rc;rc=put8(code,0xbeu);if(rc!=AMIVM_JIT_OK)return rc;rc=put32(code,arg);if(rc!=AMIVM_JIT_OK)return rc;if(two_args){rc=put8(code,0xbau);if(rc!=AMIVM_JIT_OK)return rc;rc=put32(code,second);if(rc!=AMIVM_JIT_OK)return rc;}rc=emit_call_tail(code,helper);if(rc!=AMIVM_JIT_OK)return rc;finish_control_code(block,code);return AMIVM_JIT_OK;}
