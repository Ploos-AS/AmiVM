                                     ...
                                     uint32_t value)
{
    ...
    rc = translate_long(context, new_sp, 1, physical);
    if (rc != 0) return rc;

    for (i = 0u; i < 4u; ++i) {
        const unsigned shift = (3u - i) * 8u;
        if (!amivm_write8(context->vm, physical[i],
                          (uint8_t)(value >> shift))) return -4;
    }
    context->cpu->a[7] = new_sp;
                                    ...
                                    uint32_t *value)
{
    ...
    rc = translate_long(context, context->cpu->a[7], 0, physical);
    if (rc != 0) return rc;
