...
int main(void){
 ...
 const uint16_t jmp_an[]={0x4ef5u,0xc600u|0x0020u};
 ...
 cpu.a[7]=sp;cpu.isp=sp;cpu.a[5]=0x30000000u;cpu.a[4]=5u;amivm_ir_block_init(&b,0x5000u);CHECK(amivm_ir_decode_words(&b,jmp_an,2u)==0);CHECK(b.ops[0].index_is_addr==1u);CHECK(b.ops[0].index_scale==4u);CHECK(amivm_ir_execute(&b,&cpu,&vm)==1);CHECK(cpu.pc==0x30000034u);CHECK(cpu.a[7]==sp);
