...
const uint16_t jmp_pc[]={0x4efbu,0x9310u,0xfff0u};
/* full extension with base suppression, D1.l*1, null base displacement */
const uint16_t jmp_bs[]={0x4ef4u,0x1980u};
...
cpu.a[7]=sp;cpu.isp=sp;cpu.a[1]=3u;amivm_ir_block_init(&b,0x5000u);CHECK(amivm_ir_decode_words(&b,jmp_pc,3u)==0);CHECK(amivm_ir_execute(&b,&cpu,&vm)==1);CHECK(cpu.pc==0x4ff8u);CHECK(cpu.a[7]==sp);
cpu.d[1]=0x12345678u;amivm_ir_block_init(&b,0x6000u);CHECK(amivm_ir_decode_words(&b,jmp_bs,2u)==0);CHECK(b.ops[0].base_suppress==1u);CHECK(amivm_ir_execute(&b,&cpu,&vm)==1);CHECK(cpu.pc==0x12345678u);
