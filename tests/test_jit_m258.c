...
cpu.d[2]=4u;table=AMIVM_RAM_BASE+0x8002u+0x20u+4u;/* Map the synthetic guest PC-relative table into VM RAM. */CHECK(put32(&vm,table,0x22000000u));amivm_ir_block_init(&b,AMIVM_RAM_BASE+0x8000u);CHECK(amivm_ir_decode_words(&b,pc_pre,4u)==0);CHECK(b.ops[0].ea_mode==AMIVM_IR_EA_PC_D8_XN);CHECK(b.ops[0].indirect_mode==AMIVM_EA_INDIRECT_PREINDEXED);CHECK(run(&b,&cpu,&vm)==0);CHECK(cpu.pc==0x22000010u);CHECK(cpu.a[7]==sp-4u);
cpu.a[7]=sp;cpu.isp=sp;cpu.d[1]=8u;table=AMIVM_RAM_BASE+0x9002u+0x30u;CHECK(put32(&vm,table,0x32000000u));amivm_ir_block_init(&b,AMIVM_RAM_BASE+0x9000u);CHECK(amivm_ir_decode_words(&b,pc_post,4u)==0);CHECK(b.ops[0].ea_mode==AMIVM_IR_EA_PC_D8_XN);CHECK(b.ops[0].indirect_mode==AMIVM_EA_INDIRECT_POSTINDEXED);CHECK(run(&b,&cpu,&vm)==0);CHECK(cpu.pc==0x31fffff8u);CHECK(cpu.a[7]==sp);
